...
output="/mnt/c/Users/jorsm/OneDrive/masters_thesis/VCFs/vcfs/controls/variant_stats_controls.txt"
...
for srr in "${srr_numbers[@]}"; do
    ...
    # C to U
    awk 'BEGIN {OFS="\t"} /^#/ {print; next} ($4 == "G" && $5 == "T") || ($4 == "C" && $5 == "A")' "$srr"_uncommon_snps_passes.vcf > "$srr"_CtoU.vcf

    echo "No. of C to U" >> $output
    grep AC= "$srr"_CtoU.vcf | wc -l >> $output
